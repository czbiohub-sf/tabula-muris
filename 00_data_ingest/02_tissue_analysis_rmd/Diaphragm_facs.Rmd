...
PCHeatmap(object = tiss, pc.use = 1:9, cells.use = 500, do.balanced = TRUE, 
          label.columns = FALSE, num.genes = 16)
```
...
tiss <- FindClusters(object = tiss, reduction.type = "pca", dims.use = 1:n.pcs, 
                     resolution = used.res, print.output = 0, save.SNN = TRUE, force.recalc = TRUE)
...
VlnPlot(tiss, genes_to_check, point.size.use = 0.5, nCol = 2)
...
# SC - subtypes
genes_to_check = c('Vcam1', 'Calcr', 'Itga7', 'Pax7', 'Pax3', 'Myod1', 'Myf5', 'Cd34', 'Cd44' #, 'Myog'
                   )
print (genes_to_check)
FeaturePlot(tiss, genes_to_check, pt.size = pt.size, cols.use = cols.use, nCol = 3)
VlnPlot(tiss, genes_to_check, point.size.use = 0.5, nCol = 3)
DotPlot(tiss, genes_to_check, do.return=T) + coord_flip()
```
...
VlnPlot(tiss, genes_to_check, point.size.use = 0.5, nCol = 1)
DotPlot(tiss, genes_to_check, do.return=T) + coord_flip()
```
...
cell_ontology_class <- c("skeletal muscle satellite cell", "mesenchymal stem cell", 
                        "lymphocyte", "endothelial cell", "macrophage")
free_annotation <- c("skeletal muscle satellite stem cell", "mesenchymal stem cell", 
                        "B cell", "endothelial cell", "macrophage")
...
write.csv(tiss@meta.data[,c('plate.barcode','cell_ontology_class','cell_ontology_id', 
                            'free_annotation')], file=filename)
```
