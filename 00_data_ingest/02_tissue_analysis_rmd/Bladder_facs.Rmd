...
PCHeatmap(object = tiss, pc.use = 1:10, cells.use = 500, do.balanced = TRUE, label.columns = FALSE, num.genes = 8)
```
...
genes_to_check = c('Epcam','Upk1b', 'Upk3a','Grhl3', 'Krt5', 'Krt14', 'Cd44', 'Dcn', "Car3", "Scara5")
...
```{r}
tiss.markers %>% group_by(cluster) %>% top_n(25, avg_logFC)
```
...
cell_ontology_id = c("CL:1001319", "CL:1001319", "CL_1001428", "CL_1001428", "CL_1001428", "CL_1001428", "CL:1001319")
...
for (i in 0:6) {clustermarkers = filter(tiss.markers, cluster == i)$gene
DotPlot(tiss, genes.plot = clustermarkers[1:10], plot.legend = T)}
```

```{r}
save.image("bladder_plate_020818.RData")
```
...
write.csv(tiss@meta.data[,c('plate.barcode','cell_ontology_class','cell_ontology_id')], file="Bladder_cell_ontology_class.csv")
```



##########nothing below was analyzed
