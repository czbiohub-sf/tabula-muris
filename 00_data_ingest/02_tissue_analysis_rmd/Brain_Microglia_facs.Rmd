...
tiss <- RunTSNE(object = tiss, dims.use = 1:n.pcs, seed.use = 10, perplexity=40)
```
...
```{r}
RidgePlot(tiss, genes_to_check, group.by='mouse.sex')
```
...
Look at clusters and previous figshare annotation
```{r}
...
Look at clusters and sex enrichment
```{r}
table(tiss@ident, tiss@meta.data$mouse.sex)
```


```{r}
cluster.ids <- c(0, 1, 2, 3,4,5,6)
free_annotation <- c("male enriched", "female enriched", "female enriched", "male enriched", NA, NA, NA)
cell_ontology_class <-c("microglial cell","microglial cell","microglial cell","microglial cell","microglial cell","macrophage", "microglial cell")
