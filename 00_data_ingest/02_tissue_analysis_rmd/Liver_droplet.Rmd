...
```{r}
FeaturePlot(tiss, c("Cyp2f2","Cyp2e1"))
```
...
FeaturePlot(tiss, genes_hep, pt.size = 1, nCol = 5, cols.use = c("grey", "red"))
```
...
VlnPlot(tiss, c("Hamp", "Gstp1","Cyp2f2", "Cyp2e1"), use.raw= T, nCol = 2)
```
...
```{r}
subtiss = SubsetData(tiss, ident.use = c(7,8))
```
...
```{r}
mito.genes <- grep(pattern = "^Mt", x = rownames(tiss@data), value = TRUE)
percent.mito <- Matrix::colSums(tiss@raw.data[mito.genes, ])/Matrix::colSums(tiss@raw.data)
mito.genes
tiss <- AddMetaData(tiss, percent.mito, col.name = "percent.mito")
```
...
# stash QC stats
liver <- AddMetaData(object = liver, metadata = percent.mito, col.name = "percent.mito")

```{r}
VlnPlot(object = tiss, features.plot = c("nGene", "nUMI", "percent.mito"), nCol = 3)
```

```{r}
GenePlot(subtiss, "nUMI", "percent.ribo")
```
