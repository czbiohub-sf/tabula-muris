...
cell_ontology_class = validate_cell_ontology(cell_ontology_class)
...
subtiss@meta.data['free_annotation'] <- as.character(plyr::mapvalues(x = subtiss@ident, from = cluster.ids, to = sub.free_annotation))
subtiss@meta.data['cell_ontology_class'] <- plyr::mapvalues(x = subtiss@ident, from = sub.cluster.ids, to = sub.cell_ontology_class)
subtiss@meta.data['cell_ontology_id'] <- plyr::mapvalues(x = subtiss@ident, from = sub.cluster.ids, to = sub.cell_ontology_id)
```

```{r}
DotPlot(subtiss, genes.plot = genes_to_check_TF, group.by = 'cell_ontology_class')
```
...
```{r}
FetchData(subtiss, c('cell_ontology_class','nReads')) %>% 
  ggplot(aes(nReads, colour = cell_ontology_class)) + 
  geom_freqpoly(binwidth = 2e5) 
```

```{r}
FetchData(subtiss, c('cell_ontology_class','percent.ribo')) %>% 
  ggplot(aes(percent.ribo,colour = cell_ontology_class)) + 
  geom_freqpoly(binwidth = 0.005) 
```

```{r}
FetchData(subtiss, c('cell_ontology_class','Rn45s')) %>% 
  ggplot(aes(Rn45s, colour = cell_ontology_class)) + 
  geom_freqpoly() 
```
...
```{r}
JoyPlot(object = tiss, features.plot = c("Gcg", "Ppy"), group.by='cell_ontology_id')
```
...
```
```{r, fig.width=7}
DotPlot(tiss, genes_to_check_TF, plot.legend = T)
```
