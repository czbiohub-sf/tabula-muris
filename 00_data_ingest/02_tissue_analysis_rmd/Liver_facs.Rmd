...
Sexual Dimorphism: Mup20/Darcin, Cyp2c69, Cyp2c40, Cyp2d9, Sult3a1, Sult2a1, Gstp1
...
genes_b = c('Cd79a', 'Cd79b', 'Cd74', 'Cd19')
...
genes_to_check_other = c('Pecam1', 'Nrp1', 'Kdr','Oit3','Emr1', 'Clec4f', 'Cd68', 'Irf7')
#genes_to_check_other = c('Zap70', 'Il2rb', 'Nkg7', 'Cxcr6', 'Klr1c', 'Gzma','Cd79a', 'Cd79b', 'Cd74', 'Cd19')

FeaturePlot(tiss, genes_to_check_other, pt.size = 3, nCol = 2, cols.use = c("grey", "red"))
```
...
```{r}
subtiss = SubsetData(tiss, ident.use = c(1,2,3,4,6))
```
...
```{r}
PCHeatmap(object = subtiss, pc.use = 1:3, cells.use = 100, do.balanced = TRUE, label.columns = FALSE, num.genes = 8)
PCElbowPlot(subtiss)
```
...
```{r}
TSNEPlot(subtiss, group.by = 'cluster.ids')
FeaturePlot(subtiss, 'Alb')
```


```{r, echo=FALSE, fig.height=12, fig.width=8}
FeaturePlot(subtiss, all_genes)
```
...
```{r}
markers.0 <- FindMarkers(subtiss, ident.1 = 0)
```
