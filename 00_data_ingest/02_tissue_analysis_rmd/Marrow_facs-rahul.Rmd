...
```{r}
subtissA = SubsetData(tiss, ident.use = c(7))
```
...
```{r}
PCHeatmap(object = subtissA, pc.use = 1:3, cells.use = 500, do.balanced = TRUE, label.columns = FALSE, num.genes = 8)
...
tiss = stash_subtissA_in_tiss(tiss, subtissA)
```
...
```{r}
subtissB = SubsetData(subtissB, ident.use = c(0))
```
...
tiss = stash_subtissB_in_tiss(tiss, subtissB)
```
...
```{r}
subtissC = SubsetData(tiss, ident.use = c(6))
```
...
tiss = stash_subtissC_in_tiss(tiss, subtissC)
```
...
```{r}
subtissD = SubsetData(tiss, ident.use = c(0, 10))
```
...
tiss = stash_subtissD_in_tiss(tiss, subtissD)
...
```{r}
subtissE = SubsetData(subtissE, ident.use = c(0, 2, 5, 6, 7))
```
...
tiss = stash_subtissE_in_tiss(tiss, subtissE)
...
TSNEPlot(object = subtissF, do.label = TRUE, do.return = TRUE, group.by = "cell_ontology_class")
```
