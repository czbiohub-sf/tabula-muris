...
sub.cell_ontology_id = c(NA,
  ...
  NA,
  NA)
sub.cell_ontology_class <-c(NA, NA, NA, NA, NA)
...
#sub.free_annotation <- c()
sub.cell_ontology_id = c("CL:0008001",
  NA,
  "CL:0008001",
  NA,
  NA,
...
```{r}
subsubtiss = SubsetData(subtiss, ident.use = c(0, 2, 5, 6, 7))
```
...
subsubtiss <- subsubtiss %>% FindClusters(reduction.type = "pca", dims.use = 1:subsub.n.pcs, 
    resolution = subsub.res.use, print.output = 0, save.SNN = TRUE) %>%
    RunTSNE(dims.use = 1:subsub.n.pcs, seed.use = 10, perplexity=60)
...
```{r}
subsub.cluster.ids <- c(0, 1, 2, 3, 4, 5, 6)
...
subsub.cell_ontology_id = c(
  ...
  "CL:0000557", 
  "CL:0000050")
subsub.cell_ontology_class <-c("regulatory T cell", "Slamf1-negative multipotent progenitor cell", "common lymphoid progenitor", NA, "Slamf1-positive multipotent progenitor cell", "granulocyte monocyte progenitor cell", "megakaryocyte-erythroid progenitor cell")
...
```{r}
TSNEPlot(object = subsubtiss, do.label = TRUE, do.return = TRUE, group.by = "free_annotation")
```
...
```{r}
subsubsubtiss = SubsetData(subsubtiss, ident.use = c(3))
```

```{r}
subsubsubtiss <- subsubsubtiss %>% ScaleData() %>% 
  FindVariableGenes(do.plot = TRUE, x.high.cutoff = Inf, y.cutoff = 0.5) %>%
  RunPCA(do.print = FALSE)
```

```{r}
PCHeatmap(object = subsubsubtiss, pc.use = 1:3, cells.use = 170, do.balanced = TRUE, label.columns = FALSE, num.genes = 8)
PCElbowPlot(subsubsubtiss)
```
...
subsubsub.n.pcs = 2
subsub.res.use = 1
subsubsubtiss <- subsubsubtiss %>% FindClusters(reduction.type = "pca", dims.use = 1:subsubsub.n.pcs, 
    resolution = subsub.res.use, print.output = 0, save.SNN = TRUE) %>%
    RunTSNE(dims.use = 1:subsubsub.n.pcs, seed.use = 10, perplexity=18)

TSNEPlot(object = subsubsubtiss, do.label = T, pt.size = 1.2, label.size = 4)

```
Check expression of genes of interset.

```{r}
subsubsubtiss.markers <- FindAllMarkers(object = subsubsubtiss, only.pos = TRUE, min.pct = 0.25, thresh.use = 0.25)
```

```{r}
subsubsubtiss.markers %>% group_by(cluster) %>% top_n(6, avg_logFC)
```
...
FeaturePlot(subsubsubtiss, genes_to_check, pt.size = 1, nCol = 3)
```


```{r, echo=FALSE, fig.height=8, fig.width=10}
#DotPlot(subsubsubtiss, genes_to_check, col.max = 2.5, plot.legend = T, do.return = T) + coord_flip()
```
...
```{r}
table(FetchData(subsubsubtiss, c('mouse.id','ident')) %>% droplevels())
```
...
subsubsubtiss@meta.data['free_annotation'] <- as.character(plyr::mapvalues(x = subsubsubtiss@ident, from = subsub.cluster.ids, to = subsub.free_annotation))
validate_cell_ontology(subsub.cell_ontology_class)
subsub.cell_ontology_id = convert_to_cell_ontology_id(subsub.cell_ontology_class)

subsubsubtiss@meta.data['free_annotation'] <- as.character(plyr::mapvalues(x = subsubsubtiss@ident, from = subsub.cluster.ids, to = subsub.free_annotation))
subsubsubtiss@meta.data['cell_ontology_class'] <- as.character(plyr::mapvalues(x = subsubsubtiss@ident, from = subsub.cluster.ids, to = subsub.cell_ontology_class))
subsubsubtiss@meta.data['cell_ontology_id'] <- as.character(plyr::mapvalues(x = subsubsubtiss@ident, from = subsub.cluster.ids, to = subsub.cell_ontology_id))
```

## Add subsubcluster cell_ontology_classs to main cell_ontology_class

```{r}
subsub.cells = rownames(subsubsubtiss@meta.data)

tiss@meta.data[subsub.cells, 'free_annotation'] = subsubsubtiss@meta.data[,'free_annotation']
tiss@meta.data[subsub.cells, 'cell_ontology_class'] = subsubsubtiss@meta.data[,'cell_ontology_class']
tiss@meta.data[subsub.cells, 'cell_ontology_id'] = subsubsubtiss@meta.data[,'cell_ontology_id']
```
...
Color by metadata, like plate barcode, to check for batch effects.
```{r}
TSNEPlot(object = subsubsubtiss, do.return = TRUE, group.by = "plate.barcode")
```
...
```{r}
TSNEPlot(object = subsubsubtiss, do.label = TRUE, do.return = TRUE, group.by = "free_annotation")
```
