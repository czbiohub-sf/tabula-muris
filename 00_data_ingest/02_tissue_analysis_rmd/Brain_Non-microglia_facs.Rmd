...
subtiss@meta.data['free_annotation'] <- as.character(plyr::mapvalues(x = subtiss@ident, from = sub.cluster.id, to = sub.free_annotation))
validate_cell_ontology(sub.cell_ontology_class)
sub.cell_ontology_id = convert_to_cell_ontology_id(sub.cell_ontology_class)

subtiss@meta.data['free_annotation'] <- as.character(plyr::mapvalues(x = subtiss@ident, from = sub.cluster.id, to = sub.free_annotation))
subtiss@meta.data['cell_ontology_class'] <- as.character(plyr::mapvalues(x = subtiss@ident, from = sub.cluster.id, to = sub.cell_ontology_class))
subtiss@meta.data['cell_ontology_id'] <- as.character(plyr::mapvalues(x = subtiss@ident, from = sub.cluster.id, to = sub.cell_ontology_id))
```
```
...
```{r}
table(tiss@meta.data$cell_ontology_class)
TSNEPlot(object = subtiss, do.label = T, group.by="sub.cell_ontology_class")
```
