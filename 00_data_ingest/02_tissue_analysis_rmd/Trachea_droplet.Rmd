---
 title: "Trachea Droplet Notebook"
...
tiss <- RunTSNE(object = tiss, dims.use = 1:n.pcs, seed.use = 10, perplexity=15, dim.embed = 2)
```
...
genes_to_check = c('Epcam', 'Cdh1', 'Krt5', 'Scgb1a1', 'Pdgfrb', 'Pdgfra', 'Col1a1', 'Col8a1', 'Foxj1', 'Pecam1', 'Ptprc', 'Cck')


FeaturePlot(tiss, genes_to_check, pt.size = .1)
#TRACHEA SUPP FIGURE 7
```
...
DotPlot(tiss, genes_to_check, plot.legend = T, col.min = -20, col.max = 20, dot.scale = 12, cols.use = c("green","red"))
```
...
```{r}
VlnPlot(tiss, features.plot = "Pdgfra")
#TRACHEA SUPP FIGURE 8
```

```{r}
VlnPlot(tiss, features.plot = "Pdgfrb")
#TRACHEA SUPP FIGURE 8
```

```{r}
VlnPlot(tiss, features.plot = "Col1a1")
#TRACHEA SUPP FIGURE 8
```

```{r}
VlnPlot(tiss, features.plot = "Col8a1")
#TRACHEA SUPP FIGURE 8
```

```{r}
VlnPlot(tiss, features.plot = "Ptprc")
#TRACHEA SUPP FIGURE 8
```

```{r}
VlnPlot(tiss, features.plot = "Pecam1")
#TRACHEA SUPP FIGURE 8
```

```{r}
VlnPlot(tiss, features.plot = "Epcam")
#TRACHEA SUPP FIGURE 8
```

```{r}
VlnPlot(tiss, features.plot = "Cdh1")
#TRACHEA SUPP FIGURE 8
```

```{r}
VlnPlot(tiss, features.plot = "Krt5")
#TRACHEA SUPP FIGURE 8
```

```{r}
VlnPlot(tiss, features.plot = "Foxj1")
#TRACHEA SUPP FIGURE 8
```

```{r}
VlnPlot(tiss, features.plot = "Scgb1a1")
#TRACHEA SUPP FIGURE 8
```

```{r}
VlnPlot(tiss, features.plot = "Cck")
#TRACHEA SUPP FIGURE 8
```
...
```{r}
clust.markers <- FindMarkers(object = tiss, ident.1 = 5, only.pos = TRUE, min.pct = 0.25, thresh.use = 0.25)
```


```{r}
print(x = head(x= clust.markers, n = 50))
```
...
free_annotation = c("mesenchymal 3","mesenchymal 1","mesenchymal 2",NA,NA,NA,NA)
cell_ontology_class <- c("mesenchymal cell","mesenchymal cell","mesenchymal cell","blood cell","endothelial cell","epithelial cell","neuroendocrine cell")
...
# TRACHEA SUPP FIGURE 2, WOULD BE NICE TO INCLUDE FREE_ANNOTATION LABELS
```
...
subtiss <- SubsetData(object = tiss, ident.use = c(5), do.center = F, do.scale = F)
```
...
subtiss <- FindClusters(object = subtiss, reduction.type = "pca", dims.use = 1:sub.n.pcs, 
    resolution = sub.res.used, force.recalc = TRUE ,print.output = 0, save.SNN = TRUE)
```
...
Check expression of genes of interset.
```{r}
genes_to_check = c('Krt5', 'Krt14', 'Scgb1a1', 'Foxj1')

FeaturePlot(subtiss, genes_to_check, pt.size = 1)
#TRACHEA SUPP FIGURE 9
```
...
sub.cell_ontology_class <- c("basal cell", "epithelial cell", "secretory cell", "epithelial cell","ciliated cell")
validate_cell_ontology(sub.cell_ontology_class)
sub.cell_ontology_id = convert_to_cell_ontology_id(sub.cell_ontology_class)

subtiss@meta.data[,'cell_ontology_class'] <- plyr::mapvalues(x = subtiss@ident, from = subcluster.ids, to = sub.cell_ontology_class)
subtiss@meta.data[,'cell_ontology_id'] <- plyr::mapvalues(x = subtiss@ident, from = subcluster.ids, to = sub.cell_ontology_id)
...
```{r}
library(tidyverse)
figshare_annotation = read_csv(here("00_data_ingest","01_droplet_raw_data", "annotations_droplets.csv"))
...
```{r}
table(tiss@meta.data$figshare_annotation, tiss@meta.data$ident)
```

```{r}
table(tiss@meta.data$figshare_annotation, tiss@meta.data$cell_ontology_class)
```
