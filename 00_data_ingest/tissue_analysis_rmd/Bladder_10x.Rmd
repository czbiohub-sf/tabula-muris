...
plate_metadata_filename = here('00_data_ingest', 'droplet_raw_data', 'metadata_droplet.csv')

plate_metadata <- read.csv(plate_metadata_filename, sep=",", header = TRUE)
colnames(plate_metadata)[1] <- "channel"
plate_metadata
```
...
```{r}
tissue_plates = filter(plate_metadata, tissue == tissue_of_interest)[,c('channel','tissue','subtissue','mouse.sex')]
tissue_plates
```
...
filename = here('00_data_ingest', 'droplet_raw_data', 'droplet', paste0(tissue_of_interest, '-counts.csv'))
...
```{r}
channels = lapply(colnames(raw.data), function(x) strsplit(strsplit(x, "_")[[1]][1], '.', fixed=TRUE)[[1]][2])
head(channels)
```
...
```{r}
barcode.df = t.data.frame(as.data.frame(channels))

rownames(barcode.df) = colnames(raw.data)
colnames(barcode.df) = c('channel')
head(barcode.df)

rnames = row.names(barcode.df)
meta.data <- merge(barcode.df, plate_metadata, by='channel', sort = F)
row.names(meta.data) <- rnames

# Sort cells by plate barcode because that's how the data was originally
meta.data = meta.data[order(meta.data$channel), ]
corner(meta.data)
...
genes_to_check = c('Epcam','Upk1b', 'Upk3a','Grhl3', 'Krt5', 'Krt14', 'Dcn', 'Col1a1','Col8a1', 'Pecam1', 'Cd14' )
