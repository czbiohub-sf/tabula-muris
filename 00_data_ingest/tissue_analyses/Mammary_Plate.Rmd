...
annotation <- c("basal cells", "basal cells", "luminal progenitors", "basal cells", "stromal cells", "hormone responsive luminal cells", "stromal cells", "endothelial cells", "basal cells")
...
subtiss <- SubsetData(object = tiss, ident.use = c(0,1,3), do.center = F, do.scale = F, cells.use = )
...
```{r}
subtiss.markers %>% group_by(cluster) %>% top_n(10, avg_diff)
```

Check expression of genes of interset.
```{r}
genes_to_check = c('Alpl', '1500015O10Rik', 'Rab20', 'Rnf122', 'Cebpd', 'Ptx3', 'Cxcr7', 'Tgm1', 'Slc43a3')
...
```{r}
subcluster.ids <- c(3, 6, 4,7, 5, 0)
subannotation <- c("Fos+","Cxcl2+Vcam+Procr+", "Ifgbp2+Igfbp4", "Id1+Id2+Pdfgrl+Wnt10a", "Chac1+Mthfd2+", "Chac1+Mthfd2+")
