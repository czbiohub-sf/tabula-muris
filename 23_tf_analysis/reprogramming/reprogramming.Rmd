...
# Get all cell types expressing all genes above minimum fraction
```{r}
# A.k.a "BAM"
...
# Does our data tell us anything interesting about the relationships between TF and efficiency? 
Data from Tsunemoto et al, 2018
```{r}
require(data.table)
eff.mat <- fread("Tsunemoto_reprog_efficiencies.csv")
eff.dt <- melt.data.table(eff.mat, id.vars = "V1", variable.name = "TF_1", value.name = "efficiency")
...
# Does "Neuron" rank predict efficiency?
```{r}
# First, scale each gene's fraction to 1
fractions.scaled <- fractions / apply(fractions, 2, max)
...
for(TF.duo in eff.dt[,(TF.duo)]) {
  ...
  if((tf1 == "None") && (tf2 == "None")){
    ...
    if(tf1 == "None"){
      ranks <- rank(fractions.scaled[tf2,])
    }else if(tf2 == "None"){
      ranks <- rank(fractions.scaled[tf1,])
...
The distribution of ranks is clearly bimodal, so what would their success rate have been had they taken only pairs fromt the best mode? 
```{r}
ggplot(neuron.ranks, aes(neuron.rank, ..count..))+geom_histogram()
neuron.ranks[neuron.rank < 60, duo.type := "good_duo"]
neuron.ranks[neuron.rank > 60, duo.type := "bad_duo"]
...
The tile plot above could be a really useful way to visualize how pairs of TFs generate cell type specificity. Can it be extended to 3 or 4 dimensions? Maybe 3 dimensions...