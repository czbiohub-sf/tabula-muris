...
cell.type = "bcell"
system(paste("mkdir",cell.type))
bcell.labels <- tiss@meta.data %>% filter(cell_ontology_class %like% "B.cell")
...
cell.type = "tcell"
system(paste("mkdir",cell.type))
tcell.labels <- tiss@meta.data %>% filter(cell_ontology_class %like% "T.cell")
...
make.tSNE <- function(object, cell.type, n.pcs=10, calc.PCA.only = F, subset.cells = F){
  ...
  if(subset.cells){
    ...
    object <- SubsetData(tiss, cells.use = cells.use)
  }
  ...
  object@var.genes <- object@var.genes[!tiss@var.genes %in% iegs]
...
celltype.diffexpr(data = tiss, cell.type = 'bcell', max.cells.per.ident = 100, n.cores = 4)
celltype.diffexpr(data = tiss, cell.type = 'tcell', max.cells.per.ident = 100, n.cores = 4)
```
...
corr.epith=plot.correlogram('epithelial', 1e-7, 0.3, 4)

corr.strom=plot.correlogram('stromal', 1e-7, 0.3, 8)
 
corr.bcell=plot.correlogram('bcell', 1e-7, 0.3, 8)

corr.tcell=plot.correlogram('tcell', 1e-7, 0.3, 8)
...
Calculate tSNE DR
```{r}
make.tSNE(tiss,'endothelial', n.pcs=10) # re-runs all the steps including normalization to generate the tSNE dimensionality reduction
make.tSNE(tiss,'epithelial', n.pcs=15) # re-runs all the steps including normalization to generate the tSNE dimensionality reduction
make.tSNE(tiss,'stromal', n.pcs=15) # re-runs all the steps including normalization to generate the tSNE dimensionality reduction

make.tSNE(tiss,'bcell', n.pcs=15) # re-runs all the steps including normalization to generate the tSNE dimensionality reduction
make.tSNE(tiss,'tcell', n.pcs=15) # re-runs all the steps including normalization to generate the tSNE dimensionality reduction
```


# Gene-tSNE plots
```{r}
plot.tsne.annot(tiss,'endothelial')
genes.plot <- c("Foxq1","Pbx1","Gata4","Meox2")
plot.tsne.markers(tiss, 'endothelial', genes.plot)

plot.tsne.annot(tiss,'epithelial')
genes.plot <- c("Sox4","Foxq1","Lhx2","Pax9")
plot.tsne.markers(tiss, 'epithelial', genes.plot)

plot.tsne.annot(tiss,'tcell')
genes.plot <- c("Sox4","Foxp3","Nr4a2","Eomes")
plot.tsne.markers(tiss, 'tcell', genes.plot)
# 
plot.tsne.annot(tiss,'bcell')
genes.plot <- c("Sox4","Gata3","Tbx21","Crem")
plot.tsne.markers(tiss, 'bcell', genes.plot)
