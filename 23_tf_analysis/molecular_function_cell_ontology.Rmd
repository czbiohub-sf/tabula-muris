...
tm.facs.matrix.binding.cellids <- tm.facs.matrix.binding %>% 
  group_by(factors) %>%
  summarise_at(vars(colnames(tm.facs.matrix.binding[1:1016])),funs(sum(., na.rm=TRUE)))


tm.facs.matrix.binding.cellids.norm = tm.facs.matrix.binding.cellids
for (i in 1:82){
  jaux = sum(tm.facs.matrix.binding.cellids[i,2:1017])
  for (j in 2:1017){
...
barplot(c(round(entanglement(entangle.listdend.ta),2),
              ...
              round(entanglement(entangle.listdend.tra),2)), main="Entanglements", horiz=FALSE,
        names.arg=c("Transporter Activity", "Molecular Function Regulator", "Structural Molecular Activity","Molecular Transducer Activity","Trancription Regulator Activity"),
        xlab = "Entanglement",
