...
plotgene_vln <- function(tiss, gene, tissue, method, meta="cell_ontology_class", meta_display_name="Cell Ontology Class", width=14, height=7){
  ...
  table_plot = tableGrob(summary_table, rows = NULL, theme = ttheme_default(base_size=7))
  table_plot$heights = unit(rep(1/(1.01*nrow(table_plot)), 1.01*nrow(table_plot)), "npc")
  table_plot$widths = unit.pmax(rep(table_plot$widths*1), unit(2, "lines"))
  ...
  plots <- plot_grid(vplot, table_plot, rel_widths = c(2, 1))
  ...
  plots <- plot_grid(bwplot, table_plot, rel_widths = c(2, 1))

  ggsave(here('21_website','images', paste0(tissue, "-", method, "-", gene, "-baw", ".png")), plot = plots, width = width, height = height, limitsize = FALSE)
...
plotmeta <- function(tiss, tissue, method, metas, meta_display_names, index){
  ...
  if (tissue == "All") {
  tiss %>% TSNEPlot(do.label = TRUE, pt.size = 0.5, group.by='cell_ontology_class', do.return = T) +
	  scale_colour_discrete(name = legend_name) +
	  xlim(plot_min, plot_max) + ylim(plot_min, plot_max) + coord_fixed(ratio = 1) +
	  xlab("tSNE 1") + ylab("tSNE 2")
  } else {
	  plot_data = tiss %>% TSNEPlot(group.by='cell_ontology_class', do.return = T, do.bare = T) 
      
    centers = plot_data$data %>% group_by(ident) %>% summarize(x = median(x = x), y = median(x = y))
  
    plot_data +
    geom_point(data = centers, mapping = aes(x = x, y = y), size = 0, alpha = 0) +
    geom_text(data = centers, mapping = aes(label = ident), size = 2.7) +
	  xlim(plot_min, plot_max) + ylim(plot_min, plot_max) + coord_fixed(ratio = 1) +
	  scale_colour_discrete(name = legend_name) + theme(legend.key.height = unit(1, "cm")) +
    scale_size(range = c(min(plot_data$data$pt.size), max(plot_data$data$pt.size))) +
	  xlab("tSNE 1") + ylab("tSNE 2")
  }
...
legend_name = "Cell Ontology Class"

lims = FetchData(tiss, c('tSNE_1', 'tSNE_2', meta)) %>% summarize(xmin = min(tSNE_1), xmax = max(tSNE_1), ymin = min(tSNE_2), ymax = max(tSNE_2))
plot_min = min(lims$xmin, lims$ymin) - 5
plot_max = max(lims$xmax, lims$ymax) + 5
...
plot_data +
geom_point(data = centers, mapping = aes(x = x, y = y), size = 0, alpha = 0) +
geom_text(data = centers, mapping = aes(label = ident), size = 2.5) +
xlim(plot_min, plot_max) + ylim(plot_min, plot_max) + coord_fixed(ratio = 1) +
scale_colour_discrete(name = legend_name) + theme(legend.key.height = unit(1, "cm")) +
scale_size(range = c(min(plot_data$data$pt.size), max(plot_data$data$pt.size))) +
xlab("tSNE 1") + ylab("tSNE 2")
