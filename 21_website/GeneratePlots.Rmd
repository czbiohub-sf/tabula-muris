...
plotmeta <- function(tiss, tissue, method, metas, meta_display_names, index){
  ...
  plot_data = FetchData(tiss, c('tSNE_1', 'tSNE_2', meta))

  if (tissue == "All") {
	plot_data %>% ggplot(aes_string(x = 'tSNE_1', y = 'tSNE_2', color = meta_R_safe)) +
	  geom_point(size = 0.5) +
	  scale_colour_discrete(name = legend_name) +
	  xlim(plot_min, plot_max) + ylim(plot_min, plot_max) + coord_fixed(ratio = 1) +
	  xlab("tSNE 1") + ylab("tSNE 2")
  } else {
	centers = plot_data %>% group_by(cell_ontology_class=.[, meta]) %>% summarize(x=mean(tSNE_1), y=mean(tSNE_2))
	plot_data %>% ggplot(aes_string(x = 'tSNE_1', y = 'tSNE_2', color = meta_R_safe)) +
	  #geom_point(data = centers, mapping = aes(x = x, y = y), size = 0, alpha = 0) +
	 	#geom_text(aes(label=)) +
	  #geom_text(data = centers, mapping = aes(label=seq(1, dim(centers)[1]), x=x, y=y)) +
	  geom_point(size = 0.5) +
	  xlim(plot_min, plot_max) + ylim(plot_min, plot_max) + coord_fixed(ratio = 1) +
