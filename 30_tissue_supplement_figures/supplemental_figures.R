...
ridgeplot_and_save = function(tiss, save_folder, prefix, group.by, genes,
  colors.use=NULL, i=0, n_chunks=0, suffix=''){
        if (colors.use == NULL){
...
tsne_and_legend = function(tiss,
                           save_folder,
                           prefix,
                           group.by, colors.use=NULL,  do.label = FALSE){
...
dot_tsne_ridge = function(tiss,
  ...
  for (group.by in group.bys) {
    ...
    for (i in names(chunked_genes)) {
      ...
      ridgeplot_and_save(tiss, save_folder, prefix, group.by, genes, colors.use, i, n_chunks)
      # dev.off()
    }
