...
genes_to_check = c("Ald1l1", "Aqp4", "Ascl1", "Calb1", "Cldn5", "Cspg4", "Dcx", "Des", "Dlx2", "Eno2", "Gad1", "Gjc2", "Ly6c1", "Mab21l1", "Mag", "Mcam", "Mobp", "Mog", "Mop", "Neurod6", "Ocln", "Pdgfra", "Pdgfrb", "Pecam1", "Rbfox3", "Reln", "Slc17a7", "Slc1a3", "Slco1c1", "Snap25", "Susd5")
```
...
filename = file.path(save_folder, paste('Neurons', 'cell_ontology_class', 'ridgeplot'))
...
RidgePlot(object = ECs, features.plot = genes_check, do.sort=F, size.title.use = 18, size.x.use = 0.3,size.y.use = 3, cols.use = brewer.pal(11,'Paired'))
ggsave(filename, dpi=300)
dev.off()
write_caption("Key defining genes for the Inflamed (Venous) and Notch (Arterial) populations.", filename)
```
...
filename = make_filename(save_folder, prefix, 'function_and_vessel_type', 'tsneplot')
...
```{r}
filename = make_filename(save_folder, prefix, 'function_and_vessel_type', 'featureplot')
...
filename = make_filename(save_folder, prefix, 'function_and_vessel_type', 'dotplot')
...
## Subset: "Astrocytes"```{r use-optipng, optipng=''}
Astrocytes.cells.use = rownames(tiss@meta.data)[tiss@meta.data$cell_ontology_class == "astrocyte"]
write(paste("Number of cells in Astrocytes subset:", length(Astrocytes.cells.use)), stderr())
Astrocytes.n.pcs = 10
Astrocytes.res.use = 1
Astrocytes.perplexity = 30
Astrocytes.genes_to_check = c("A2m", "Aldh1l1", "Aqp4", "Gdf10", "Gfap", "Naga", "Nbl1", "Nsdhl", "Slc1a3", "St6galnac5", "Tnfaip2", "Vim")
Astrocytes.tiss = SubsetData(tiss, cells.use=Astrocytes.cells.use, )
Astrocytes.tiss <- Astrocytes.tiss %>% ScaleData() %>% 
  FindVariableGenes(do.plot = TRUE, x.high.cutoff = Inf, y.cutoff = 0.5) %>%
  RunPCA(do.print = FALSE)
Astrocytes.tiss <- Astrocytes.tiss %>% FindClusters(reduction.type = "pca", dims.use = 1:Astrocytes.n.pcs, 
    resolution = Astrocytes.res.use, print.output = 0, save.SNN = TRUE) %>%
    RunTSNE(dims.use = 1:Astrocytes.n.pcs, seed.use = 10, perplexity=Astrocytes.perplexity)

```

### Append this subset's groupby to the list```{r use-optipng, optipng=''}
...
tiss@meta.data[, "Astrocytes"] = "(Not in subset)"
tiss@meta.data[Astrocytes.cells.use, "Astrocytes"] = "Astrocytes" 
filename = make_filename(save_folder, prefix="Astrocytes", 'highlighted', 
    'tsneplot_allcells')
p = TSNEPlot(
  object = tiss,
  do.return = TRUE,
...
dot_tsne_ridge(Astrocytes.tiss, Astrocytes.genes_to_check,
    save_folder, prefix = "Astrocytes", group.bys, "facs")
