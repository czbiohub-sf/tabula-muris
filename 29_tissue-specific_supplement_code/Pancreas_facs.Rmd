Figure 1.
```{r}
filename = file.path(save_folder, paste('Delta_cells','Sst','expression', 'cell_ontology_class', 'ridgeplot.pdf', sep='_'))
...
filename = file.path(save_folder, paste('Delta_cells', 'Neurog3_x_Hhex', 'scaled', 'mouse.id', 'scatterplot.pdf', sep='_'))
ggplot(delta.1, aes(x=Hhex, y=Neurog3, color = mouse.id)) + geom_point() + geom_rug(sides = "bl")
ggsave(filename, dpi=300)
dev.off()

#2b - using raw data.
filename = file.path(save_folder, paste('Delta_cells', 'Neurog3', 'raw', 'cell_ontology_class', 'ridgeplot.pdf', sep='_'))
...
filename = file.path(save_folder, paste('Delta_cells', 'Neurog3_x_Hhex', 'raw', 'Neurog3_expression', 'scatterplot.pdf', sep='_'))
ggplot(delta.2, aes(Hhex, Neurog3, color = Neurog3 > 0))+ geom_point() + geom_rug(sides="bl")
ggsave(filename, dpi=300)
dev.off()

filename = file.path(save_folder, paste('Neurog3_x_Hhex', 'raw', 'mouse.id', 'scatterplot.pdf', sep='_'))
...
Figure 3.
```{r}
filename = file.path(save_folder, paste('Delta_cells', 'Prss53', 'expression', 'tsneplot.pdf', sep='_'))
